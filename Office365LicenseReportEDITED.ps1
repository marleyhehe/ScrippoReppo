...
Function ConnectMgGraphModule 
{
    ...
    if($null -eq $MsGraphModule) # I CHANGED HERE
    { 
...
Function Get_UsersLicenseInfo
{
    ...
    foreach($Sku in $SKUs)  #License loop
    {
        ...
        foreach($Service in $Services) #Service loop
        {
            ...
            $ServiceFriendlyName = $ServiceArray|Where-Object{$_.Service_Plan_Name -eq $ServiceName} #I CHANGED HERE
            ...
            $Result  | Export-Csv -Path $ExportCSV -Notype -Append -Encoding UTF8
        }
     ...
     $Output=[PSCustomObject]@{'Displayname'=$_.Displayname;'UserPrincipalName'=$UPN;Country=$Country;CompanyName=$CompanyName;'LicensePlanWithEnabledService'=$LicensePlanWithEnabledService;'FriendlyNameOfLicensePlanAndEnabledService'=$FriendlyNameOfLicensePlanWithService}
     $Output | Export-Csv -path $ExportSimpleCSV -NoTypeInformation -Append -Encoding UTF8
}
...
Function main()
{
    ...
    if($UserNamesFile -ne "")
    {
        ...
        {
            Get-MgUser -All -UserId $item.UserPrincipalName |Where-Object{$null -ne $_.AssignedLicenses} | ForEach-Object{ # I CHANGED HERE
    ...
    {
        Get-MgUser -All|Where-Object{$null -ne $_.AssignedLicenses} | ForEach-Object{ # I CHANGED HERE
        Get_UsersLicenseInfo
        $LicensedUserCount++}
    }
